...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    event.preventDefault();
    console.log(Object.values(formErrorValues));
    if (Object.values(formErrorValues).length > 0) {
      console.log("Error found");
    } else {
      let res = await api.post("/api/insertUser", { ...formValues }, config);
      console.log(res.data);
      if (res.data === "Data already exist") {
        alert("The above user is already registered");
        navigate("/signup");
  ...
  const Validation = (formValues) => {
    ...
    }else if (!dobpattern.test(formValues.dob)) {
